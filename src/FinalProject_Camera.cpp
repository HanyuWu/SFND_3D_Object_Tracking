...
int main(int argc, const char *argv[]) {
  ...
  // Decide detectorType
  string detectorType;
  std::cout << "Please enter a type of detector\n"
            << "SHITOMASI, HARRIS, FAST, BRISK, ORB, AKAZE or SIFT "
            << std::endl;
  std::cin >> detectorType;

  // Decide descriptorType
  string descriptorType;
  std::cout << "Please enter a type of descriptor\n"
            << "BRISK, BRIEF, ORB, FREAK, AKAZE or SIFT" << std::endl;
  std::cin >> descriptorType; // BRIEF, ORB, FREAK, AKAZE, SIFT
  ...
  for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex;
    ...
    detectObjects((dataBuffer.end() - 1)->cameraImg,
                  ...
                  yoloModelConfiguration, yoloModelWeights, true);
        ...
        1) // wait until at least two images have been processed
    {
      ...
      string descriptorDataType =
          (descriptorType == "SIFT" || descriptorType == "FAST" ||
